...
class Arm:
    ...
    def __init__(self) -> None:
        ...
        self.M1=move.Motor(0)
        self.M2 = move.Motor(1)
        self.M3 = move.Motor(2)
        self.RANGES=[[0,4095]]
        self.Sent_Positions=[0]
        self.motors=[self.M1]
        for m in self.motors:
            m.Disable_Torque()
            m.SetCurrentLimit(1750)
            m.SetAccelLimit(500)
    ...
    def set_range(self, id, min, max):
        ...
        Returns:
            void
        """
        r0=self._map(min,0,360,0,4095)
        r1=self._map(max,0,360,0,4095)
        self.RANGES[id][0]=r0
        self.RANGES[id][1]=r1
    ...
    def set_Ranges(self,ranges):
        ...
        for i, r in enumerate(ranges):
            r0=self._map(r[0],0,360,0,4095)
            r1=self._map(r[1],0,360,0,4095)
            self.RANGES[i][0]=r0
            self.RANGES[i][1]=r1
    ...
    def get_Pos(self):
        ...
        """
        # self.EnableTorque()
        self.M1_angle=int(self._map(self.M1.Read_Pos(),0,4095,0,360))
        self.angles=[self.M1_angle]
        for idx,angle in enumerate(self.angles):
            if angle>360:
                self.angles[idx] = angle-4095 ###
        return self.angles
    ...
    def set_Pos(self,angles):
        ...
        for idx in range(len(self.motors)):
            pos = int(self._map(angles[idx],0,360,0,4095))
            #print(pos)
    ...
    def set_motor_pos(self,id,angle):
        pos=int(self._map(angle,0,360,0,4095))
    ...
    def set_Pos_order(self,angles,order):
        ...
        for m in order:
            real_pos=int(self._map(self.motors[m-1].Read_Pos(),0,4095,0,360))
            pos=int(self._map(angles[m-1],0,360,0,4095))
    ...
    def SetXYZWithoutOrientation(self,position, q_init):
        ...
        my_chain = Chain(name='my_chain', links=[
            OriginLink(),
            URDFLink(name="link1", bounds=(0, 360), translation_vector=[0, 0, 135], orientation=[1, 0, 0], rotation=0),
        ])
    ...
    def SetXYZ(self,position, orientation, q_init):
        ...
        my_chain = Chain(name='my_chain', links=[
            OriginLink(),
            ModifiedURDFLink(name="link1", bounds=(0, 360), origin_translation=[0, 0, 135], origin_orientation=np.eye(3), rotation=q_init[0]),
        ])
