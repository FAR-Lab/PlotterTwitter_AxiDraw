...
class Motor:
    ...
    The dynamixel motor parameters
        ADDR_TORQUE_ENABLE         -> 0 ~ 1
        ADDR_GOAL_POSITION         -> Min Position Limit(52) ~ Max Position Limit(48)
        ADDR_PRESENT_POSITION
        ADDR_DXL_MAX_VELOCITY
        ADDR_DXL_MAX_Accel
        ADDR_DXL_Current_LIMIT
        DXL_MINIMUM_POSITION_VALUE -> Refer to the Minimum Position Limit of product eManual
        DXL_MAXIMUM_POSITION_VALUE -> Refer to the Maximum Position Limit of product eManual
        LEN_GOAL_POSITION          -> Data Byte Length
        LEN_PRESENT_POSITION       -> Data Byte Length
        BAUDRATE                   -> 9,600 [bps] ~ 4 [Mbps]
        # DYNAMIXEL Protocol Version (1.0 / 2.0)
        # https://emanual.robotis.com/docs/en/dxl/protocol2/
        PROTOCOL_VERSION           -> 2.0

        # Factory default ID of all DYNAMIXEL
        DXL_ID                     -> 0 ~ 252
        ...
        DXL_MOVING_STATUS_THRESHOLD = 0 ~ 1,023  -> Dynamixel moving status threshold
    """
    def __init__(self, id) -> None:
        ...
        self.ADDR_DXL_Current_LIMIT = 1750
        ...
        self.P_Gain_pos                  = 400       # gain of the motor controller
        self.I_Gain_pos                  = 0         # integrator gain of the motor
        self.D_Gain_pos                  = 400       # diverse gain of the motor
        self.P_Gain_vel                  = 40       # gain of the motor controller
        self.I_Gain_vel                  = 400         # integrator gain of the motor
        BAUDRATE                         = 57600
        # DYNAMIXEL Protocol Version (1.0 / 2.0)
        # https://emanual.robotis.com/docs/en/dxl/protocol2/
        PROTOCOL_VERSION                 = 2.0
        ...
        self.groupwrite_num = GroupSyncWrite(self.packetHandler, PROTOCOL_VERSION, self.ADDR_GOAL_POSITION, self.LEN_GOAL_POSITION)

        # Initialize Groupsyncread Structs for Present Position
        self.groupread_num = GroupSyncRead(port=self.packetHandler, ph=PROTOCOL_VERSION, start_address=self.ADDR_PRESENT_POSITION,
                                      data_length=self.LEN_PRESENT_POSITION)
    ...
    def SetCurrentLimit(self,value = 1750):
        """
        Enter Current limit in mA
        """
        for i in self.DXL_ID:
            dxl_comm_result, dxl_error = self.packetHandler.write2ByteTxRx(self.portHandler, i, self.ADDR_DXL_Current_LIMIT, value)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def GetCurrentLimit(self):
        ...
        """
        for i in self.DXL_ID:
            self.dxl_current_limit[i], dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler,
                                                                                                  i,
                                                                                                  self.ADDR_DXL_Current_LIMIT)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
            return self.dxl_current_limit[i]

    def SetVelocityLimit(self, value = 1620):
        """
        Enter Velocity limit in rev/min
        """
        for i in self.DXL_ID:
            dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, i,
                                                                           self.ADDR_DXL_MAX_VELOCITY, value)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def GetVelocityLimit(self):
        ...
        """
        for i in self.DXL_ID:
            self.dxl_current_limit[i], dxl_comm_result, dxl_error = self.packetHandler.read4ByteTxRx(self.portHandler, i,
                                                                                                self.ADDR_DXL_MAX_VELOCITY)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def SetAccelLimit(self, value = 500):
        """
        Enter Acceleration limit in rev/min^2
        """
        for i in self.DXL_ID:
            dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, i,
                                                                           self.ADDR_DXL_MAX_Accel, value)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def GetPresentCurrent(self):
        ...
        """
        for i in self.DXL_ID:
            self.dxl_present_current[i], dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler, i,
                                                                                                    self.ADDR_DXL_Present_current)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
            return self.dxl_present_current[i]

    def Close_Port(self):
        self.portHandler.closePort()

    def Write_Pos(self, angle, DXL_ID):
    ...
    def Read_Pos(self, DXL_ID):
        ...
        self.dxl_present_position, dxl_comm_result, dxl_error = self.packetHandler.read4ByteTxRx(self.portHandler,
                                                                                                 DXL_ID,
    ...
    def Homeing(self):
        ...
        while not finished:
            for i in self.DXL_ID:
                self.Enable_Torque()
                self.Write_Pos(pose, i)
                if not self.inRange(self.Read_Pos(i),i):
                    raise Exception("Out of range, Homing failed, Try moving the arm and homing again")
                if (round(self.Read_Pos(i)*(360/4095)) - pose) < 2 : flag.append(True)
            if flag:
                finished = True



    def GoTo(self, pose, id):
        finished = False
        while not finished:
            self.Enable_Torque()
            self.Write_Pos(pose, id)
            if not self.inRange(self.Read_Pos(id),id):
                raise Exception("Out of range, Homing failed, Try moving the arm and homing again")
                if (round(self.Read_Pos(i)*(360/4095)) - pose) < 2 : flag.append(True)
            if flag:
                finished = True
    ...
    def inRange(self,value,id):
        if value > 4095:
                value = value-4095 #####
        return ((self.DXL_MAXIMUM_POSITION_VALUE>value) and (self.DXL_MINIMUM_POSITION_VALUE<value))

# The following set of functions represent attributes of the robot



    def Pointing(self):
        self.Enable_Torque()
        self.Write_Pos(270, 1)
        if not self.inRange(self.Read_Pos(2), 2):
...
class AxiDraw(axidraw.AxiDraw):
    ...
    def music_motor(self):
        '''Interactive context: The motor play music '''
        ebb_motion.doLowLevelMove()